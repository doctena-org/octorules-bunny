...
class TestBunnyLint:
    ...
    def test_phase_filter(self):
        rules_data = {
            "bunny.waf_custom_rules": [{"action": "block"}],
            "bunny.waf_rate_limit_rules": [{"action": "block"}],
        }
        ctx = _ctx(phase_filter={"bunny.waf_rate_limit_rules"})
        bunny_lint(rules_data, ctx)
        phases = {r.phase for r in ctx.results}
        assert "bunny.waf_custom_rules" not in phases
