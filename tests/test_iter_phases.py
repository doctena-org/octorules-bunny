...
class TestIterPhases:
    def test_yields_bunny_phases(self):
        data = {
            "bunny.waf_custom_rules": [{"ref": "a"}],
            "bunny.waf_rate_limit_rules": [{"ref": "b"}],
        }
        results = list(iter_provider_phases(data, _ctx(), BUNNY_PHASE_NAMES))
        names = [name for name, _ in results]
        assert "bunny.waf_custom_rules" in names
        assert "bunny.waf_rate_limit_rules" in names

    def test_skips_non_bunny_phases(self):
        data = {
            "cf_waf_custom_rules": [{"ref": "a"}],
            "bunny.waf_custom_rules": [{"ref": "b"}],
        }
        results = list(iter_provider_phases(data, _ctx(), BUNNY_PHASE_NAMES))
        names = [name for name, _ in results]
    ...
    def test_skips_non_list_values(self):
        data = {
            "bunny.waf_custom_rules": "not-a-list",
        }
        results = list(iter_provider_phases(data, _ctx(), BUNNY_PHASE_NAMES))
    ...
    def test_respects_phase_filter(self):
        ...
        results = list(
            iter_provider_phases(
                data, _ctx(phase_filter={"bunny.waf_custom_rules"}), BUNNY_PHASE_NAMES
            )
        )
        names = [name for name, _ in results]
        assert "bunny.waf_custom_rules" in names
        assert "bunny.waf_rate_limit_rules" not in names

    def test_skip_suffixes(self):
        data = {
            "bunny.waf_custom_rules": [{"ref": "a"}],
            "bunny.waf_access_list_rules": [{"ref": "b"}],
            "bunny.edge_rules": [{"ref": "c"}],
        }
        results = list(
            iter_provider_phases(
        ...
        names = [name for name, _ in results]
        assert "bunny.waf_custom_rules" in names
        assert "bunny.waf_access_list_rules" not in names
        assert "bunny.edge_rules" not in names
